...
describe('TESTANDO NUMEROS INTEIROS', () => {

  it("circle(1); // retorno: {radius: 1, area: 3.14, circumference: 6.28}", () => {
...
it("circle(7); // retorno: {radius: 7, area: 153.86, circumference: 43.96}", () => {
  expect(circle(7)).toEqual({ radius: 7, area: 153.86, circumference: 43.96 })
});

/*- circle(3) // Retorno: {radius: 3, area: 28,26, circumference: 18.84}*/

it("circle(3); // retorno: {radius: 3, area: 28.26, circumference: 18.84}", () => {
...
describe('4 - Implemente os casos de teste para a função `circle`', () => {
  it('Verifica se ao receber um raio, a função `circle` retorna um objeto contendo os valores esperados', () => {
    ...
    expect(circle(1)).not.toBeUndefined();

    // Teste se circle retorna um objeto.

    expect(circle()).not.toBe('Object');
